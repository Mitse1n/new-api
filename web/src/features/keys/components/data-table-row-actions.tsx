...
function PersonalDataTableRowActions<TData>({
  row,
...
export function DataTableRowActions<TData>(props: { row: Row<TData> }) {
  ...
  return orgID === null ? (
    <PersonalDataTableRowActions {...props} />
  ) : (
    <OrganizationDataTableRowActions {...props} />
  )
}

function OrganizationDataTableRowActions<TData>(props: { row: Row<TData> }) {
  const { t } = useTranslation()
