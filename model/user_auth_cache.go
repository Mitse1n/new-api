...
redis.call('HSET', KEYS[1],
  ...
  'Setting', ARGV[8], 'AuthVersion', ARGV[1], 'CacheSchema', ARGV[9], 'PersonalOrgId', ARGV[13])
...
return 1`
	result, err := common.RDB.Eval(context.Background(), script,
		...
		user.Username, user.Setting, user.CacheSchema, includeQuotaArg, user.Quota, ttl, user.PersonalOrgId,
	).Int()
	if err != nil {
		return err
	}
