...
> 2026-09-08 更新：本文保留原始设计背景；其中“个人组织”和存量个人资源回填方案已取消。当前个人资源直接归属用户，钱包使用 User.Quota，只有真实团队创建 Organization。当前实现以 multi-tenancy-implementation.md 为准。
...
- **OrganizationInvite**：用户名站内邀请（绑定已有账号 ID），通知中心支持接受与拒绝，不生成邀请链接。
...
- 组织层（新增，存 `OrganizationMember.Role`）：
  ...
  |---|---|---|---|---|---|---|
  | `owner` | 管理 | 仅自己创建的 | 查看 | ✅ | 管理 | 转让、停用 |
  | `admin` | 管理 | 仅自己创建的 | 查看 | ✅ | 管理 | ❌ |
  | `billing` | ❌ | ❌ | 查看 | ✅ | 查看 | ❌ |
...
// model/organization_invite.go — 用户名站内邀请，绑定账号 ID + 过期时间 + 状态
```
...
4. 平台管理员的跨租户查询走**显式独立函数**（组织账单、用量查询等），而不是「orgID == 0 就不过滤」的隐式分支——隐式分支是越权漏洞的温床。
...
| 6 | 成员被移除后其 Key 与历史用量归属 | 已确认：Key 仅创建者可管理，组织统一付费；停用/移除成员会禁用其 Key，历史用量保留 |
...
- 启动时批量禁用旧数据中非活跃或无成员关系的组织 Key；不删除记录。Token 缓存升级到 `token:org-v2:`，避免旧进程留下的活跃状态被读取。部署应停止旧实例并统一切换版本。
